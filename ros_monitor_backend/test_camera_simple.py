...
class SimpleCameraTester:
    ...
    def _image_callback(self, msg: Image):
        ...
        try:
            ...
            if cv_image is not None:
                # 保存图像到文件
                filename = f"test_image_{self.camera_id}_{self.frame_count:04d}.jpg"
    ...
    def _create_test_image(self, width: int, height: int):
        ...
        try:
            ...
            cv2.putText(img, f'Encoding: {self.topic}', (10, height//2 + 80), 
    ...
    def shutdown(self):
        ...
        try:
            ...
            logger.info(f"相机测试器关闭: {self.topic}")
        except Exception as e:
            logger.error(f"关闭相机测试器时发生错误 {self.topic}: {e}")

def main():
    """主函数"""
    logger.info("启动简化的相机测试器...")
    
    # 初始化ROS节点
    rospy.init_node('simple_camera_tester', anonymous=True)
    ...
    for topic in camera_topics:
        tester = SimpleCameraTester(topic)
        testers.append(tester)
    
    logger.info("相机测试器启动完成，等待图像数据...")
